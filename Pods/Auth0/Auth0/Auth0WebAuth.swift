    ...
    final class Auth0WebAuth: WebAuth {
        ...
        var state: String {
            return parameters["state"] ?? generateDefaultState()
        }
        ...
        func connectionScope(_ connectionScope: String) -> Self {
            parameters["connection_scope"] = connectionScope
            return self
        }
        ...
        func start(_ callback: @escaping (WebAuthResult<Credentials>) -> Void) {
            guard let redirectURL = redirectURL, let urlScheme = redirectURL.scheme else {
            ...
            if let invitationURL = invitationURL {
                guard let queryItems = URLComponents(url: invitationURL, resolvingAgainstBaseURL: false)?.queryItems,
                      ...
                      let invitationId = queryItems.first(where: { $0.name == "invitation" })?.value
                else {
            ...
            let authorizeURL = buildAuthorizeURL(withRedirectURL: redirectURL,
        ...
        func clearSession(federated: Bool, callback: @escaping (WebAuthResult<Void>) -> Void) {
            let endpoint = federated ?
                URL(string: "v2/logout?federated", relativeTo: url)! :
                URL(string: "v2/logout", relativeTo: url)!
            ...
            guard let logoutURL = components?.url,
                  ...
                  let urlScheme = redirectURL.scheme
            else {
                return callback(.failure(WebAuthError(code: .noBundleIdentifier)))
            }
        ...
        {
            let authorize = URL(string: "authorize", relativeTo: url)!
            ...
            parameters.forEach { entries[$0] = $1 }
        ...
        private func handler(_ redirectURL: URL) -> OAuth2Grant {
            var authentication = Auth0Authentication(clientId: clientId,
                                                     url: url,
                                                     session: session,
                                                     telemetry: telemetry)
