    ...
    struct PKCE: OAuth2Grant {
        ...
        func credentials(from values: [String: String], callback: @escaping (WebAuthResult<Credentials>) -> Void) {
            ...
            authentication
                ...
                .start { result in
                    switch result {
                    case let .failure(error) where error.localizedDescription == "Unauthorized":
                        return callback(.failure(WebAuthError(code: .pkceNotAllowed)))
                    case let .failure(error): return callback(.failure(WebAuthError(code: .other, cause: error)))
